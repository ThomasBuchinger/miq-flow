    ...
    class Docker
      ...
      def import(tmpdir, miq_domain)
        commands =  [
          ...
          "docker exec #{@container_name} /bin/bash --login -c 'rake evm:automate:import DOMAIN=#{miq_domain} IMPORT_DIR=#{tmpdir}/automate OVERWRITE=true PREVIEW=false ENABLED=true'"
        ]
