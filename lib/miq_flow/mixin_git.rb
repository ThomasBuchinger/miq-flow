...
module MiqFlow
  ...
  module GitMethods
    def self.clone_repo(opts)
      ...
      raise MiqFlow::GitError, "Failed to clone repository at #{url}: #{e}"
    ...
    def self.local_repo(opts)
      ...
      raise MiqFlow::GitError, "Failed to find a repository at #{path}" if $git_repo.nil?
    end
    ...
    def get_diff_paths(base=nil, head=nil)
      ...
      raise MiqFlow::GitError, 'Diff failed: base is nil'   if base.nil?
      raise MiqFlow::GitError, 'Diff failed: branch is nil' if head.nil?
